...
```
- G function is probability of how likely nearest neighbors are to occur(?)
K shows what is the pattern of points wihtin a given scal (how clustered of dispersed are they)
...
plot(K_poisson, . ~ r) # The further away you go, the number of points wihtin the radius increases quadratically.
...
# at low radius's, the y-value (the k) is larger than the maximum value of the simulated values.

# Repeat for regular data
K_regular_env <- envelope(p_regular, Kest, correction = "border")
plot(K_regular_env, . - pi * r ^ 2 ~ r)
# poisson process = the line is close to 0 for all r values (radius's)
```
...
quadrat.test(redoak, alternative = "regular") # p-value = 1
quadrat.test(redoak, alternative = "clustered") # very significant p-value: p-value = 4.477e-07

# My answer: I can reject the null hypothesis with p-value less than 0.01
```
